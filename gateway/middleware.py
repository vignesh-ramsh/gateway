...
(see gateway/__init__.py): security headers -> CORS -> CSRF -> identity.
...
def identity_middleware(authn_provider: Any) -> Middleware:
    """Only ever composed into the pipeline if kernel.has("authn") at
    register() time (see gateway/__init__.py) — this function itself has
    no conditional logic because the condition already happened once,
    structurally, at boot. Resolves identity once per request and attaches
    it to scope["state"]["arc_identity"], which gateway's dispatcher reads
    onto Request.identity."""
